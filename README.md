# Proyecto-Máquina-Expendedora-Empotrados
The purpose of this code is to create a simple implementation of a coffee machine that can be interacted with using a joystick and reprogrammed on the spot using an admin portal.
## Hardware
...
10. Arduino UNO:
	...
	You can also use a MEGA board or similar, but any board smaller than a UNO won't have enough pins to follow this tutorial and will require more tinkering.
...
### Wiring
We recommend wiring and testing each component individually using the sources at the end of this README, and slowly adding components to the breadboard to avoid confusion.
Be sure to check the variables in the program and wire accordingly.

## Software
This program works by using a state machine and some variables to keep track of the position it should be in. This allows the main loop to cycle through myself quickly and reduce latency.
...
```cpp
  switch (selectedOption) {
	case 1:
  	verTemperatura();
  	selectedOption = 0; //Set to zero to avoid reentry to the switch
  	break;
	case 2:
  	verDistancia();
  	selectedOption = 0;
...
And if the button is pressed, we start "serving" the coffee, while slowly brightening up the LED. At this point in the code, we can also add an extra water pump to serve liquid as I mention in the video.
But for the time being it's outside of the scope for this project.
```cpp
  if (bValue == LOW) {
...
A watchdog reset has been added to each while loop so that it doesn't mistakenly trigger a reset if you spend too much time on a menu, but it can still reset if an error occurs during execution.
```cpp
...
Move the joystick to the right to cycle between products, change prices moving it up and down, and discard changes moving it to the left. The modifications are confirmed by pressing the joystick button.
